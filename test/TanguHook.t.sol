...
import {TanguHook} from "../src/TanguHook.sol";
import {IAavePool} from "../src/interfaces/IAavePool.sol";

contract TestTanguHook is Test, IERC721Receiver {
    ...
    function setupPool(address usdc, address usdt) public {
        ...
        tanguHook = new TanguHook{salt: salt}(poolManager, aavePool);

        console2.log("tanguHook:", address(tanguHook));

        tanguHook.setATokens(address(aUSDC), Currency.wrap(usdc));
        tanguHook.setATokens(address(aUSDT), Currency.wrap(usdt));
        ...
        poolKey = PoolKey({
            ...
            tickSpacing: 200,
            hooks: IHooks(address(tanguHook))
        });
        ...
        params[0] = abi.encode(poolKey, -200, 200, 1_000_000e6, type(uint).max, type(uint).max, address(this), "");
